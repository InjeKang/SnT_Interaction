...
library(reticulate)
conda_list(conda="auto") ## anaconda prompt랑 똑같은데..??

# Load Python
use_condaenv(condaenv = "jiseong", required = TRUE) ######여기서부터 에러
# py_install("pandas")

# Load data
data = data.table(py_load_object('./data/cleansing/data_final'))[,c(1:4)]
colnames(data) = c("type", "year", "text_raw", "text_cleansed")
head(data)
...
# 토픽 레이블링
# topic_labels = paste0(paste0("T", stm_effect_model$topics, "-"), c("지방자치&규제", "경제", "정치(선거)", "대기업", "모빌리티갈등", 
#                                                                    "자본시장", "정치(국회)", "정치(연설)", "노사관계", "인문사회", 
#                                                                    "국제무역", "외교", "교육(일반)", "중소벤처기업"))
...
for (i in 1:num_topic) {
  ...
  doc_date = data.frame(index_by_topic, types_by_topic, years_by_topic, docs_by_topic, topic_name)
  colnames(doc_date) = c("index", "type", "year", "text", "topic")
